...
class CustomUserCreationForm(UserCreationForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)  # UserCreationForm 초기화
    ...
    def save(self, commit=True):
        ...
        # student_number와 같은 추가 필드를 설정
        user.student_number = self.cleaned_data.get('student_number')
        user.student_card_image = self.cleaned_data.get('student_card_image')
        # 추가 필드를 모두 설정한 후 최종 저장
        if commit:
            user.save()
